...
// 		});
	...
	function fn_save() {
		...
		$("#EMAIL_DISP_TIME").attr("disabled",false);
		
		jQuery.ajax({
			url           : '${staticPATH }/setContentsChannelEmail.do',
			dataType      : "JSON",
			scriptCharset : "UTF-8",
			type          : "POST",
	        data          : $("#form").serialize(),
...
//		if($("input:radio[name='EMAIL_EDIT_YN']:checked").val() == "Y"){
  console.log("#EMAIL_EDIT_YN : " + $("#EMAIL_EDIT_YN").val());
    if($("#EMAIL_EDIT_YN").val() == "Y"){
			...
			$("#EMAIL_FROMNAME").attr("readonly",true);
			$("#EMAIL_FROMADDRESS").addClass("essentiality");
			$("#EMAIL_FROMADDRESS").attr("readonly",true);
			$("#EMAIL_REPLYTO").addClass("essentiality");
			$("#EMAIL_REPLYTO").attr("readonly",true);
			$("#EMAIL_SUBJECT").addClass("essentiality");
			$("#EMAIL_SUBJECT").attr("readonly",true);
			$("#search").hide();
			$('#TR_CONTENT').css('display', 'none');
			$('#TD_CONTENT').attr("rowspan","1");
			$('li:eq(2)').hide();
...
<!-- 
 ...
 --><input type="hidden" name="EMAIL_EDIT_YN" id="EMAIL_EDIT_YN" class="txt"  value="Y"/>
				<tr>
				...
				<tr>
					...
					<td class="tbtd_content" colspan="3"><input type="text" id="EMAIL_SUBJECT" name="EMAIL_SUBJECT" style="width:500px;" value="${bo.email_subject}" class="txt" maxlength="200"/></td>
				</tr>				
				<tr>
