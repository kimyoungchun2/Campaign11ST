...
<script language="JavaScript">
	...
	$(document).ready(function(){
		var code_id = "${bo_dtl.code_id}";
		
		//상세 조회일경우 code_id 수정 불가
		if(code_id != null && code_id !="" ){
			$("#CODE_ID").addClass("essentiality");
	...
	function fn_save() {
		...
		jQuery.ajax({
			url           : '${staticPATH }/setCommCodeSlave.do',
			dataType      : "JSON",
			scriptCharset : "UTF-8",
			type          : "POST",
	        data          : $("#form").serialize(),
  ...
  <div class="row" style="width:100%; height100%;">
    ...
    <div class="col-lg-10">
      ...
      <form name="form" id="form">
		...
		<input type="hidden" id="CCOMM_CODE_ID"  name="CCOMM_CODE_ID" value="${CCOMM_CODE_ID}" />	
	    <div id="table">
		  <table width="100%" class="table"border="1" cellpadding="0" cellspacing="0" bordercolor="#D3E2EC" bordercolordark="#FFFFFF">
			...
			<tr>
			  <th class="info">상위 코드ID</th>
			  <td class="tbtd_content" colspan="3">
				<input type="text" id="COMM_CODE_ID" name="COMM_CODE_ID" style="width:100px;" class="essentiality" readonly="readonly"  value="${bo.comm_code_id}" />
			  </td>
			</tr>
			<tr>
			  <th class="info">상위 코드명</th>
			  <td class="tbtd_content" colspan="3">
			 	<input type="text" id="COMM_CODE_NAME" name="COMM_CODE_NAME" style="width:250px;" class="essentiality" readonly="readonly"  value="${bo.comm_code_name}"/>
		      </td>
			</tr>
			<tr>
			  <th class="info">코드ID</th>
			  <td class="tbtd_content" colspan="3">
				<input type="text" id="CODE_ID" name="CODE_ID" style="width:100px;" value="${bo_dtl.code_id}" maxlength="20" class="txt"/>
			  </td>
			</tr>
			<tr>
			  <th class="info">코드명</th>
			  <td class="tbtd_content" colspan="3">
				<input type="text" id="CODE_NAME" name="CODE_NAME" style="width:250px;" value="${bo_dtl.code_name}" maxlength="200" class="txt"/>
			  </td>
			</tr>
			<tr>
			  <th class="info">코드설명</th>
			  <td class="tbtd_content" colspan="3">
				<textarea id="CODE_DESC" name="CODE_DESC" cols="50" rows="3">${bo_dtl.code_desc}</textarea>
			  </td>
			</tr>
			<tr>
			  <th class="info">정렬순서</th>
			  <td class="tbtd_content" colspan="3">
				<input type="text" id="SORT_SEQ" name="SORT_SEQ" style="width:80px;" value="${bo_dtl.sort_seq}" onkeydown="javascript:keypressNumber();" maxlength="6" class="txt"/>
			  </td>
			</tr>			
			<tr>
			  ...
			  <td class="tbtd_content" colspan="3">
				<select id="USE_YN" name="USE_YN" >
				  <option value="Y" <c:if test="${bo_dtl.use_yn eq 'Y' }">selected="selected"</c:if>>Y</option>
				  <option value="N" <c:if test="${bo_dtl.use_yn eq 'N' }">selected="selected"</c:if>>N</option>
				</select>
			  </td>
			</tr>
			<tr>
			  <th class="info">등록자</th>
			  <td class="tbtd_content">${bo_dtl.create_nm}</td>
			  <th class="info">등록일시</th>
			  <td class="tbtd_content">${bo_dtl.create_dt}</td>
			</tr>
			<tr>
			  <th class="info">수정자</th>
			  <td class="tbtd_content">${bo_dtl.update_nm}</td>
			  <th class="info">수정일시</th>
			  <td class="tbtd_content">${bo_dtl.update_dt}</td>
			</tr>			
		  </table>
	    </div>	
